...
gantt
    ...
    Telegram Bot Assíncrono & Webhook :mil8, after mil7, 3d
    Histerese Anti-Flapping & Auto-Recovery :mil9, after mil8, 2d
    Injeção de Falhas & Testes E2E :mil10, after mil9, 3d
    section Fase 4: Piloto & Campo
    Case 3D DIN & Deploy VPS Produção :mil11, after mil10, 3d
    Instalação Piloto na Granja :mil12, after mil11, 4d
    Validação Cruzada com Sensores Silo :mil13, after mil12, 5d
```
...
- [ ] **3.3 Suíte de Testes & Simulador E2E:**
...
## 🚜 Fase 4: Piloto em Campo & Validação de Negócio
...
- [ ] **4.1 Preparação de Hardware:**
  ...
  - Gravação de firmware versão `v1.0.0-release` nos dispositivos de teste.

- [ ] **4.2 Instalação Piloto:**
  - Instalação no aviário da granja piloto junto ao Gateway Dragino.
  - Provisionamento via Captive Portal Wi-Fi no local através de smartphone do técnico.
...
## 🧠 Integração Contínua com Graphify

O grafo de conhecimento do projeto é mantido atualizado na pasta [`graphify-out/`](file:///home/brunoconter/Documentos/1_C.VALE/2%20-%20PROJETOS/16_Keepalive_Foresight/graphify-out/).

### Comandos de Utilização:
- **Consultar o grafo:**
  ```bash
  python3 -m graphify query "<sua pergunta sobre o projeto>"
  ```
- **Visualizar grafo interativo:** Abrir [`graphify-out/graph.html`](file:///home/brunoconter/Documentos/1_C.VALE/2%20-%20PROJETOS/16_Keepalive_Foresight/graphify-out/graph.html) no navegador.
- **Atualizar após novos códigos:**
  ```bash
