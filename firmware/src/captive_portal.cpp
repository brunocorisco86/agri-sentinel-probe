...
void CaptivePortal::handleSave() {
    if (_server.hasArg("ssid") && _server.arg("ssid").length() > 0) {
        ...
        String cloud = _server.hasArg("cloud_url") ? _server.arg("cloud_url") : "http://179.197.73.80:8016/api/v1/telemetry";
        strncpy(_pendingConfig.cloud_url, cloud.c_str(), sizeof(_pendingConfig.cloud_url) - 1);
        
        String token = _server.hasArg("api_token") ? _server.arg("api_token") : "keepalive-secret-token-123";
...
<body>
    <div class="container">
        ...
        <form action="/save" method="POST">
            ...
            <input type="text" id="cloud_url" name="cloud_url" value="http://179.197.73.80:8016/api/v1/telemetry">
